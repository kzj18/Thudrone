...
class ControllerNode:
    ...
    def __init__(self, test=False):
        ...
        self.window_y_list_ = [0, 1] # 窗户中心点对应的y值
    ...
    def PULL_UP(self):
        ...
        '''
        if t_wu[2] < self.safe_height - 0.15:
    ...
    def go(self,x,y,z,speed,phi):
        ...
        return False
    '''
    def go(self, x, y, z, speed=100, acc = 0):
        if acc == 0:
            acc = self.navigating_position_accuracy
        self.publishCommand('go {} {} {} {} m12'.format(int(100*x), int(100*y), int(100*z), int(speed) ))
        '''
        while 1:
            print('wait')
            if self.get_ok() == True:
        ...
        if dl < np.sqrt(2)*acc:
            ...
            return False        
    def curve(self, x_1, y_1, z_1, x, y, z, speed = 60, acc = 0):
        if acc == 0:
            acc = self.navigating_position_accuracy
        self.publishCommand('curve {} {} {} {} {} {} {} m12'.format(int(100*x_1), int(100*y_1), int(100*z_1), int(100*x), int(100*y), int(100*z), int(speed) )) 
        [x_n, y_n, z_n] = t_wu
        dx = x_n - x
        dy = y_n - y
        dz = z_n - z
...
#**************************************************************************************************************************
            ...
            if self.go(-2.3, self.window_y_list_[self.win_index], 1.65, 100) == False:
                return
            ...
            if self.BAll_flag == 0:
                ...
                if self.fire_position is not None:
                    ...
                    if self.fire_position[0] > circle_target['x'] + circle_target['err_x']:
                        dx = int(abs((self.fire_position[0] - circle_target['x'])*0.1))
                        #win_y = 0.20
                        self.gen_cmd('right', dx)
                        return
                    elif self.fire_position[0] < circle_target['x'] - circle_target['err_x']:
                        dx = int(abs((self.fire_position[0] - circle_target['x'])*0.1))
                        #win_y = -0.20
                        self.gen_cmd('left', dx)
                        return
                    if self.fire_position[1] > circle_target['y'] + circle_target['err_y']:
                        dz = int(abs((self.fire_position[1] - circle_target['y'])*0.1))
                        self.gen_cmd('down', dz)
                        #win_z = 1.75
                        return
                    ...
                    self.BAll_flag += 1
                    if self.go(-2.3, self.win_y_d, self.win_z_d,100,acc=0.1 ) == True:
                        self.BAll_flag += 1
                ...
                if t_wu[0] > -1:
                    ...
                    return
                if t_wu[0] < -0.7:
                    self.go(-0.8, self.win_y_d, self.win_z_d)
                    '''
...
#**************************************************************************************************************************
            ...
            if self.win_index == 1 and self.BAll_flag == 0:
                rospy.logwarn('win index 1')
                rospy.logwarn('st0')
                if self.go(-0.5, 0, 1.6) == True:
                    self.BAll_flag += 1
            elif self.win_index == 1 and self.BAll_flag == 1:
                if self.go( 0.6, -1.1, 1.5) == True:
                    self.BAll_flag += 1
            elif self.win_index == 0 and self.BAll_flag == 0:
                rospy.logwarn('win index 0')
                rospy.logwarn('st0')
                if self.go(0.6, -1.1, 1.5, 100) == True:
                    self.BAll_flag += 2
            elif self.BAll_flag == 2:
                rospy.logwarn('st2')
    ...
    def test_mode(self):
        ...
        if a == '2':
            ...
            with open(txt_path,'a') as f:
                ...
                f.write('###############################')
                f.write('\n')
    def publishCommand(self, command_str,sleep = 0.5):
        msg = String()
